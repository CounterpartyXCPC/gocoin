...
func (db *BchBlockDB) addToCache(h *bch.Uint256, bl []byte, str *bch.BchBlock) (crec *BlckCachRec) {
	...
	crec = &BlckCachRec{LastUsed: time.Now(), Data: bl, BchBlock: str}
...
func (db *BchBlockDB) BchBlockInvalid(hash []byte) {
	...
	if !ok {
		db.mutex.Unlock()
		println("BCH BlockInvalid: no such block", bch.NewUint256(hash).String())
		return
	}
	if cur.trusted {
...
func (db *BchBlockDB) BchBlockTrusted(hash []byte) {
	...
	if !ok {
		db.mutex.Unlock()
		println("BchBlockTrusted: no such block")
		return
	}
	if !cur.trusted {
